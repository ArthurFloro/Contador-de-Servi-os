...
export default function Gestao(){
    ...
    const incrementarEnvio = () => setEnvio(envio + 1)
    ...
    const diminuirEnvio = () => setEnvio((prev) => Math.max(prev - 1, 0));

    const resetarMatriculas = () => setMatriculas(0)
    const resetarTelefone = () => setTelefone(0)
    const resetarWhatsapp = () => setWhatsapp(0)
    const resetarEnvio = () => setEnvio(0)
    

    return(
        <>
        <div
        className="bg-white flex items-center justify-center w-3/4 m-auto py-8 
         mt-8 rounded-lg">
        <h1 className="text-orange-500 text-8xl font-bold ">Central de Atendimento</h1>
      </div>
      ...
      <div className="flex items-center justify-center mt-10 ">
        <ul className="list-disc pl-5 text-orange-500 text-3xl space-y-4">
          <li className="flex items-center space-x-4 gap-1">
            ...
            <button className="bg-orange-500 text-white px-2 py-0 rounded hover:bg-orange-600
          ...
          <li className="flex items-center space-x-4 gap-1 ">
            <span className="text-white">Envio Lista de Envio: {envio}</span>
            <button className="bg-orange-500 text-white px-2 py-0 rounded
            hover:bg-orange-600
            transition" onClick={incrementarEnvio}>+</button>

            <button className="bg-orange-500 text-white px-2 py-0 rounded
            hover:bg-orange-600
            transition" onClick={diminuirEnvio}>-</button>

            <button className="bg-orange-500 text-white px-2 py-0 rounded
            hover:bg-orange-600
            transition" onClick={resetarEnvio}>delete</button>
          </li> 
        </ul>
      </div>
